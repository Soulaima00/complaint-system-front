...
export class ClaimComponent implements OnInit {
  ...
  addClaim(){
    if (this.addClaimForm.valid) {
      ...
      const idCre = localStorage.getItem("vi2");
const idCreParsed = idCre && idCre !== "" ? Number.parseInt(idCre) : 0;
alert("idCreParsed:"+ idCreParsed);
const nom=localStorage.getItem('vn');
const fullname = nom && nom !== "" ?localStorage.getItem('vn') : " ";

this.reclamationService.create(
  this.addClaimForm.value.message,
  this.addClaimForm.value.sousTypeReclamation,
  fullname as string,
  idCreParsed,
  this.image
